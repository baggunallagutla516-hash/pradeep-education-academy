...
const links = [
  ...
  { to: '/admin/posts', label: 'Posts', icon: FileImage },
    ...
    >
      <div className={cn('mb-3 flex items-center', expanded ? 'gap-2.5 px-1' : 'justify-center')}>
        <Link
          ...
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-ink-900 text-white shadow-soft"
        >
      ...
      <div className={cn('mt-auto flex flex-col gap-2', expanded ? '' : 'items-center')}>
        ...
        >
          {expanded ? (
            <>
              <ChevronsLeft className="h-5 w-5 shrink-0" strokeWidth={1.85} />
              <span className="text-sm font-semibold">Collapse</span>
            </>
          ) : (
            <ChevronsRight className="h-5 w-5" strokeWidth={1.85} />
          )}
        </button>
        <button
          type="button"
