...
export function AdminSlipTestResultsPage() {
  ...
  async function handleRelease() {
    ...
    const released = data?.slipTest?.resultsReleased;
    if (pending <= 0) return;
    ...
    try {
      ...
      setData((prev) =>
        prev
          ? {
              ...prev,
              slipTest: res.data.data.slipTest || {
                ...prev.slipTest,
  ...
  async function handleReattempt(row) {
    ...
    if (
      !window.confirm(
        `Let ${name}${role} write this slip test again? Their current score is removed until they submit a new attempt.`
      )
    ) {
      return;
    }
    ...
    setActionInfo('');
    try {
      const res = await api.reattemptSlipTest(id, row.id);
      setActionInfo(res.data.message || `${name} can write this slip test again.`);
      const refreshed = await api.slipTestResults(id);
  ...
  async function handleExport() {
    ...
    setExporting(true);
    try {
      const slug = (data.slipTest?.title || 'slip-test')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-|-$/g, '')
        .slice(0, 40);
      downloadWorkbook(resultsExportRows(data.results), {
        sheetName: 'Results',
        fileName: `${slug || 'slip-test'}-results-${new Date().toISOString().slice(0, 10)}.xlsx`,
      });
    } catch (err) {
  ...
  async function handleToggleHide(row) {
    ...
    setActionInfo('');
    try {
      const res = await api.setSlipTestAttemptVisibility(id, row.id, hide);
  ...
  async function handleDelete(row) {
    ...
    if (
      !window.confirm(
        `Delete ${name}'s result everywhere? This cannot be undone. They can submit again only if the slip test is still open.`
      )
    ) {
      return;
    }
    ...
    setActionInfo('');
    try {
      const res = await api.deleteSlipTestAttempt(id, row.id);
  ...
  const released = Boolean(data?.slipTest?.resultsReleased);
  ...
  return (
    <PageShell
      embedded
      eyebrow="Slip test results"
      title={data?.slipTest?.title || 'Results'}
      description={
        data?.slipTest
          ? `${classLabel(data.slipTest)} · ${data.slipTest.chapter} · ${formatMarks(data.slipTest.totalMarks)} marks`
      ...
      {status === 'ready' ? (
        <>
          <div className="mb-4 flex flex-wrap items-center gap-2">
            ...
            {released && data.slipTest.resultsReleasedAt ? (
              <span className="text-xs text-ink-900/50">
                Last announced {formatDate(data.slipTest.resultsReleasedAt)}
              </span>
            ) : (
              <span className="text-xs text-ink-900/50">
          ...
          {data.results.length === 0 ? (
            <EmptyState
              ...
              description={
                data.slipTest.isPublished
                  ? 'Students have not attempted this slip test yet.'
                  : 'This slip test is still a draft, so students cannot see it.'
              }
              icon={BarChart3}
              action={
          ...
          <p className="mt-4 text-xs text-ink-900/50">
            ...
            before the student pressed submit. Reattempt clears the score while open; Hide keeps the
