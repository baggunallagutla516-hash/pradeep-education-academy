...
export function AdminCetFormPage() {
  ...
  function validate() {
    ...
    if (!form.chapterNumber.trim()) {
      next.chapterNumber = 'Chapter number is required.';
    } else if (!Number.isInteger(chapter) || chapter < 1 || chapter > 200) {
      next.chapterNumber = 'Enter a whole number between 1 and 200.';
    }
  ...
  if (loading) {
    return (
      <PageShell embedded title={isEdit ? 'Edit C.E.T.' : 'New C.E.T.'}>
  ...
  return (
    <PageShell
      ...
      description="Set when students can download the paper. Pick the classes and chapter, then upload the Chapter End Test question paper as a PDF or Word file."
      actions={
        ...
        <form className="space-y-4" onSubmit={handleSubmit} noValidate>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-ink-900/50">
              Optional. Cards show a C.E.T. icon when no image is uploaded.
            </p>
          ...
          <Button type="submit" loading={saving}>
            {isEdit ? 'Save changes' : 'Create C.E.T.'}
          </Button>
        </form>
      </Card>
    </PageShell>
