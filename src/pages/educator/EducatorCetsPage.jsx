...
export function EducatorCetsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await educatorApi.cets(params);
  ...
  return (
    <PageShell
      ...
      eyebrow="Practice"
      title="C.E.T."
      description="Published Chapter End Test papers for all classes. Filter by class if you want a shorter list."
      actions={
    ...
    >
      ...
      {status === 'ready' && cets.length === 0 ? (
        <EmptyState
          ...
          description={
            classFilter
              ? `No published C.E.T. papers for ${
                  classes.find((c) => c.id === classFilter)?.name || 'this class'
                } yet.`
              : 'No published C.E.T. papers are available yet. Check back soon.'
          }
          icon={ScrollText}
        />
      ) : null}
